...
export function DepositScreen({ onBack, onDeposit }) {
  ...
  const handleDeposit = () => {
    setLoading(true)
    setTimeout(() => { setLoading(false); onDeposit(parseInt(amount.replace(/,/g,'')) || 0) }, 2500)
  }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.inputGroup}>
        <div style={styles.inputLabel}>NETWORK</div>
        <div style={styles.channelGrid}>
          <button onClick={() => setNetwork('mtn')} style={{ ...styles.channelBtn, borderColor: network === 'mtn' ? '#F5C842' : 'var(--border)' }}>MTN</button>
          <button onClick={() => setNetwork('airtel')} style={{ ...styles.channelBtn, borderColor: network === 'airtel' ? '#FF3B5C' : 'var(--border)' }}>AIRTEL</button>
        </div>
      </div>
      ...
      <button onClick={handleDeposit} style={styles.ctaBtn} disabled={loading}>
        {loading ? 'WAITING FOR PIN...' : 'DEPOSIT NOW'}
      </button>
...
const styles = {
  ...
  ctaBtn: { padding: '20px', background: 'var(--gold)', color: '#000', border: 'none', borderRadius: 24, fontWeight: 900, fontSize: 18, cursor: 'pointer', marginTop: 12 },
